...
| Player disconnects after the app sits backgrounded for a while | Playback silently dead; only fix found so far is a page refresh | Partly closed. `not_ready`/`ready` is the connection-state event that entry asked about: the adapter now tracks the live device ID instead of one captured at init, so the next round reports `connection_lost` and offers Retry rather than accusing the deck. Still open: if the SDK never re-fires `ready`, Retry keeps failing and a reload is the only fix |
| Host loses the network mid-game — a tunnel, a lift, a dead spot | A card is burnt on a song nobody heard | Closed. A connection failure is distinguished from a market restriction and offers Retry, which replays the same card. See [tech/spotify-constraints.md](tech/spotify-constraints.md#why-a-play-command-failed--and-whether-retrying-is-honest) |
